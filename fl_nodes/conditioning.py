...
class FL_HeartMuLa_Conditioning:
    ...
    def create_conditioning(
        ...
        # Resolve muq_dim (handle lazy loading)
        muq_dim = pipeline._muq_dim
        if muq_dim is None:
            # Try to read from config.json without loading weights
            try:
                config_path = os.path.join(pipeline.heartmula_path, "config.json")
                if os.path.exists(config_path):
                    with open(config_path, 'r') as f:
                        config_data = json.load(f)
                        muq_dim = config_data.get("muq_dim", 512)
